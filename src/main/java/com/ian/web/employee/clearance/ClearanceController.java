...
public class ClearanceController {
	...
	public String saveClearance(
		...
		if (errors.hasErrors()) {
			...
			model.addAttribute("clearanceList", clearanceRepository.findByEmployeeId(clearance.getEmployee().getId()));
			return "employee/clearance/clearance-list";
		} 
		...
		if (request.getServletPath().equalsIgnoreCase("/addMyClearance")) {
			...
			return "redirect:/myclearance/"+clearance.getEmployee().getId()+"/"+clearance.getEmployee().getEmpHashCode();
		} else {
